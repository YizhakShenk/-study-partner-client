...
import {
  ...
  weekIsEmpty,
  getOptionalsDays,
} from "../../utilities/validetion/validateDate";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (editPost) {
          const subArray = result.data?.find(
            (item) => item.name === editPost.category
          );
          ...
          setTimeFrom(
            dayjs(
              new Date().setHours(
                parseInt(editPost.time_from / 100),
                parseInt(editPost.time_from % 100),
                0,
                0
              )
  ...
  const handlePost = async () => {
    try {
      ...
      if (dTo < dFrom) {
        handleOpenAlert(
          "error",
          "The end date can not be earlier from the start"
        );
        return;
      }
      if (tFrom > tTo) {
      ...
      if (daysDistance(dFrom, dTo) > 7) {
        handleOpenAlert(
          "error",
          "You can have maximum a 7 days range between the days"
        );
        return;
      }
      if (weekIsEmpty(days)) {
        ...
        <Dialog open={open} onClose={() => setOpen(false)}>
          <Box>
            ...
            <Box m={1}>
              <DialogTitle>
                {opanAlert ? (
                  <Box mb={4} sx={{ position: "relative", width: "100%" }}>
                    <Alert
                      onClose={() => setOpanAlert(false)}
                  ...
                  <Typography variant="body4">{"Post"}</Typography>
                )}
              </DialogTitle>
            </Box>
            <DialogContent>
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <Box sx={{ textAlign: "center" }}>
                  ...
                  >
                    <Box sx={{ width: "100%", maxWidth: "250px", m: 1 }}>
                      <Autocomplete
                        ...
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            label={
                              editPost !== null ? editPost.category : "Category"
                            }
                          />
                        )}
                      />
                      ...
                      <Autocomplete
                        ...
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            label={
                              editPost ? editPost.sub_category : "Sub Category"
                            }
                          />
                        )}
                      />
