...
export default function AddSubject({ setAddSubject }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const categoryOBJ = await (
          await axios.get(`${urlServer}/category/get-all`)
        ).data;
        if (!categoryOBJ) {
  ...
  const handleSave = async () => {
    try {
      if (!subject) {
        ...
        await axios.post(`${urlServer}/user-subject/add`, {
          userId: user.id,
  ...
  return (
    <Box>
      ...
      {cat ? (
        <Box sx={{ m: 2 }}>
          <Typography sx={{ m: 1 }} variant="h6" color="primary">
            Please select Subject to add
          </Typography>
          <FormControl sx={{ m: 1, minWidth: 180 }}>
            <InputLabel>Category</InputLabel>
            <Select label="Category" onChange={handleChangeCat} value={catID}>
              {cat &&
                cat.map((item, index) => {
                  return (
